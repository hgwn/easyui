	...
	$.fn.uploadFile = function(options) {
		...
		uploader.bind('Error',function(up,errObject){
			if(errObject.code == plupload.FILE_SIZE_ERROR){
				artDialog.alert("选择的文件大小超出限制，文件不大于："+uploader.getOption("filters").max_file_size);
		...
		function initProgress(){
			...
			if(!progress.length){
				var html = "<div id='file_upload_progress' style='position: fixed;top:45%;left:50%;margin-left:-100px;width:200px;'><div class='progress progress-striped' data-percent='0%'><div class='progress-bar progress-bar-success' style='width: 0%;'></div></div>"
				$("body").append(html);
			}else{
